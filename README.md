# Telegram AI Bot (OpenAI)

Telegram-бот с ИИ-ассистентом на базе OpenAI (GPT) и поддержкой обработки фотографий.

## Возможности

- 💬 Отвечает на текстовые сообщения через OpenAI
...
3. Скопируйте `.env.example` в `.env` и заполните переменные:
   ...
   - `OPENAI_API_KEY` — получить в [платформе OpenAI](https://platform.openai.com/api-keys)
...
Модель задаётся переменной `OPENAI_MODEL` в `.env`:
- `gpt-4o-mini` — быстрая и дешёвая (по умолчанию), поддерживает фото
- `gpt-4o` — более качественная, тоже поддерживает фото
